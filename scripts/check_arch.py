...
The release names each asset weav-<os>-<arch> from the build matrix, and that
name is the only thing a user has to go on when choosing a download. The name
comes from the matrix; the architecture comes from whatever machine the runner
label actually resolved to. Nothing tied the two together.

v0.2.0 shipped an arm64 binary as weav-macos-amd64, because macos-14 and
...
whatever that runner turns out to be. Only the finished artifact shows it.
    ...
    python3 scripts/check_arch.py --expect amd64 dist/weav-macos-amd64
...
Ported from phabfive, which hit the same bug in its v0.10.0 release and wrote
this in response to weav's report of it.
"""
...
def elf_arch(header: bytes) -> str:
    ...
    try:
        ...
        raise UnreadableError(f"unknown ELF e_machine 0x{machine:x}") from unknown
...
def macho_arch(header: bytes) -> str:
    ...
    try:
        ...
        raise UnreadableError(f"unknown Mach-O cputype 0x{cputype:x}") from unknown
...
def pe_arch(header: bytes) -> str:
    ...
    if header[offset : offset + 4] != b"PE\x00\x00":
        raise UnreadableError("no PE signature where the DOS header points")

    machine = little(header[offset + 4 : offset + 6])
    try:
        return PE_MACHINES[machine]
    except KeyError as unknown:
        raise UnreadableError(f"unknown PE machine 0x{machine:x}") from unknown
...
def architecture(path: str) -> str:
    ...
    if len(header) < 64:
        raise UnreadableError(f"only {len(header)} bytes long")
    ...
    raise UnreadableError(f"not an ELF, Mach-O or PE executable: {header[:4]!r}")


def main(argv: list) -> int:
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("--expect", required=True, choices=sorted(set(ELF_MACHINES.values())))
