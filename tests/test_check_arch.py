...
Each release asset is named weav-<os>-<arch> from the build matrix, while its
architecture comes from whatever machine the runner label resolved to. Nothing
tied the two together, and v0.2.0 shipped an arm64 binary named
weav-macos-amd64: macos-14 and macos-latest are both arm64 labels, so the row
meant for Intel Macs built on Apple silicon. Downloading it on an Intel Mac
...
Ported from phabfive along with the script.
"""
...
from check_arch import UnreadableError, architecture, main
...
def test_the_v0_2_0_mismatch_is_refused(tmp_path):
    """An arm64 Mach-O published under the amd64 name -- the bug v0.2.0 shipped."""
